...
{
  //块消息发送
  block_info_pub=enemy_nh_.advertise<std_msgs::Float64MultiArray>("block_info2",100);
  ...
  car1_info=enemy_nh_.subscribe("my_robort_two_pose", 1,&ArmorDetectionNode::car1Info_sendCallback, this);
  image_transport::ImageTransport video_it(enemy_nh_);
  video_pubs_ = video_it.advertise("video1_img", 1, true);
   
  //ros::Subscriber rev_tracking_sub = enemy_nh_.subscribe("return_detection", 1000, tracking_Callback); 
...
void ArmorDetectionNode::ExecuteLoop(const unsigned int index) 
{
	...
	while(running_) 
	{
		...
		if (node_state_ == NodeState::RUNNING) 
		{
			...
			cv::Point3f target_3d3;//块目标点2信息
			int num=0;
			ErrorInfo error_info = armor_detector_[index]->DetectArmor_01(video_img, detected_enemy_, target_3d1,target_3d2,target_3d3,num,msg1);  
