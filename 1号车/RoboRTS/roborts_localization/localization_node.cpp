#include "localization_node.h"
#include <std_msgs/Float64MultiArray.h>
#include <ros/ros.h>
#include<iostream>
#include<fstream>
#include "re_vi.h"

using namespace std;


//输出语句用 ROS_INFO_STREAM（）;
static int m=0;
namespace roborts_localization{
  

LocalizationNode::LocalizationNode(std::string name) {
  CHECK(Init()) << "Module "  << name <<" initialized failed!";
  initialized_ = true;
std::cout<<"11111"<<std::endl;
  //
}




bool LocalizationNode::Init() {
  
   std::cout<<"执行init"<<std::endl;


  LocalizationConfig localization_config;
  localization_config.GetParam(&nh_);

  odom_frame_   = std::move(localization_config.odom_frame_id);
  global_frame_ = std::move(localization_config.global_frame_id);
  base_frame_   = std::move(localization_config.base_frame_id);

  laser_topic_ = std::move(localization_config.laser_topic_name);

  init_pose_ = {localization_config.initial_pose_x,
                localization_config.initial_pose_y,
                localization_config.initial_pose_a};
  init_cov_ = {localization_config.initial_cov_xx,
               localization_config.initial_cov_yy,
               localization_config.initial_cov_aa};
 std::cout<<"init读参"<<std::endl;
  transform_tolerance_  = ros::Duration(localization_config.transform_tolerance);
  publish_visualize_ = localization_config.publish_visualize;
  std::cout<<"publish_visualize_ is :"<<(int)publish_visualize_<<std::endl;
  tf_broadcaster_ptr_ = std::make_unique<tf::TransformBroadcaster>();
  tf_listener_ptr_ = std::make_unique<tf::TransformListener>();

  distance_map_pub_ = nh_.advertise<nav_msgs::OccupancyGrid>("distance_map", 1, true);
  particlecloud_pub_ = nh_.advertise<geometry_msgs::PoseArray>("particlecloud", 2, true);
 //std::cout<<"init发布"<<std::endl;
  // Use message filter for time synchronizer (laser scan topic and tf between odom and base frame)
  laser_scan_sub_ = std::make_shared<message_filters::Subscriber<sensor_msgs::LaserScan>>(nh_, laser_topic_, 100);
  laser_scan_filter_ = std::make_unique<tf::MessageFilter<sensor_msgs::LaserScan>>(*laser_scan_sub_,
                                                                                   *tf_listener_ptr_,
                                                                                   odom_frame_,
                                                                                   100);
  laser_scan_filter_->registerCallback(boost::bind(&LocalizationNode::LaserScanCallback, this, _1));

  initial_pose_sub_ = nh_.subscribe("initialpose", 2, &LocalizationNode::InitialPoseCallback, this);
  pose_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("amcl_pose", 2, true);
// std::cout<<"init订阅"<<std::endl;
  amcl_ptr_= std::make_unique<Amcl>();
  amcl_ptr_->GetParamFromRos(&nh_);
  amcl_ptr_->Init(init_pose_, init_cov_);

  map_init_ = GetStaticMap();
  laser_init_ = GetLaserPose();
  ///hahahah
  vosion_location_sub = nh_.subscribe("vision_tolocation",2,&LocalizationNode::vision_location_Callback, this);

  return map_init_&&laser_init_;
}

bool LocalizationNode::GetStaticMap(){
  static_map_srv_ = nh_.serviceClient<nav_msgs::GetMap>("static_map");
  ros::service::waitForService("static_map", -1);
  nav_msgs::GetMap::Request req;
  nav_msgs::GetMap::Response res;
  if(static_map_srv_.call(req,res)) {
    LOG_INFO << "Received Static Map";
    amcl_ptr_->HandleMapMessage(res.map, init_pose_, init_cov_);
    first_map_received_ = true;
    return true;
  } else{
    LOG_ERROR << "Get static map failed";
    return false;
  }
}

bool LocalizationNode::GetLaserPose() {
  auto laser_scan_msg = ros::topic::waitForMessage<sensor_msgs::LaserScan>(laser_topic_);

  Vec3d laser_pose;
  laser_pose.setZero();
  //ROS_ERRO_STREAM("GetLaserPose");
  GetPoseFromTf(base_frame_, laser_scan_msg->header.frame_id, ros::Time(), laser_pose);
  laser_pose[2] = 0; // No need for rotation, or will be error
  DLOG_INFO << "Received laser's pose wrt robot: "<<
            laser_pose[0] << ", " <<
            laser_pose[1] << ", " <<
            laser_pose[2];

  amcl_ptr_->SetLaserSensorPose(laser_pose);
  return true;
}

void LocalizationNode::InitialPoseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &init_pose_msg) {

  if (init_pose_msg->header.frame_id == "") {
    LOG_WARNING << "Received initial pose with empty frame_id.";
  } // Only accept initial pose estimates in the global frame
  else if (tf_listener_ptr_->resolve(init_pose_msg->header.frame_id) !=
           tf_listener_ptr_->resolve(global_frame_)) {
    LOG_ERROR << "Ignoring initial pose in frame \" "
              << init_pose_msg->header.frame_id
              << "\"; initial poses must be in the global frame, \""
              << global_frame_;
    return;
  }

  // In case the client sent a pose estimate in the past, integrate the
  // intervening odometric change.
  tf::StampedTransform tx_odom;
  try {
    ros::Time now = ros::Time::now();
    tf_listener_ptr_->waitForTransform(base_frame_,
                                       init_pose_msg->header.stamp,
                                       base_frame_,
                                       now,
                                       odom_frame_,
                                       ros::Duration(0.5));
    tf_listener_ptr_->lookupTransform(base_frame_,
                                      init_pose_msg->header.stamp,
                                      base_frame_,
                                      now,
                                      odom_frame_, tx_odom);
  }
  catch (tf::TransformException &e) {
    tx_odom.setIdentity();
  }
  tf::Pose pose_new;
  tf::Pose pose_old;
  tf::poseMsgToTF(init_pose_msg->pose.pose, pose_old);
  pose_new = pose_old * tx_odom;

  // Transform into the global frame
  DLOG_INFO << "Setting pose " << ros::Time::now().toSec() << ", "
            << pose_new.getOrigin().x() << ", " << pose_new.getOrigin().y();

  Vec3d init_pose_mean;
  Mat3d init_pose_cov;
  init_pose_mean.setZero();
  init_pose_cov.setZero();
  double yaw, pitch, roll;
  init_pose_mean(0) = pose_new.getOrigin().x();
  init_pose_mean(1) = pose_new.getOrigin().y();
  pose_new.getBasis().getEulerYPR(yaw, pitch, roll);
  init_pose_mean(2) = yaw;
  init_pose_cov = math::MsgCovarianceToMat3d(init_pose_msg->pose.covariance);

  amcl_ptr_->HandleInitialPoseMessage(init_pose_mean, init_pose_cov);
}



void LocalizationNode::LaserScanCallback(const sensor_msgs::LaserScan::ConstPtr &laser_scan_msg_ptr){

  last_laser_msg_timestamp_ = laser_scan_msg_ptr->header.stamp;

  Vec3d pose_in_odom;
  if(!GetPoseFromTf(odom_frame_, base_frame_, last_laser_msg_timestamp_, pose_in_odom))
  {
    LOG_ERROR << "Couldn't determine robot's pose";
    return;
  }

  double angle_min = 0 , angle_increment = 0;
  sensor_msgs::LaserScan laser_scan_msg = *laser_scan_msg_ptr;
  TransformLaserscanToBaseFrame(angle_min, angle_increment, laser_scan_msg);

  amcl_ptr_->Update(pose_in_odom,
                    laser_scan_msg,
                    angle_min,
                    angle_increment,
                    particlecloud_msg_,
                    hyp_pose_);

  LOG_ERROR_IF(!PublishTf()) << "Publish Tf Error!";

  if(publish_visualize_){
    PublishVisualize();
  }

}

float sight_x,distance_vision,sight_z;
float arfa,sita,gama;
int figure=-1;
static int k=0;
int fig[5];
void LocalizationNode::vision_location_Callback(const std_msgs::Float64MultiArray::ConstPtr& msg)
{  
  //标签坐标=sight+hy+
   //amcl预测标签坐标
   float amcl_sign_x;
   float amcl_sign_y;
   float er_x,er_y;  
   int s_toward;
   
 ////hahaha读标签取均值//////////////
// if(k<5)
//   {
//     fig[k]=msg->data[2];
//     std::cout<<"k="<<k<<"fig"<<fig[k]<<std::endl;
//     k++;
//   }
//   else
//   {
//     k=0;
//     int i,g;
//     int a[]={0};
//     int max=-1;
//     float er_x,er_y;
//     for(g=0;g<5;g++)
//     { std::cout<<"fig="<<fig[g]<<std::endl;
//       a[fig[g]]+=1;
//       if(a[fig[g]]>max)
//       {max=a[fig[g]];figure=fig[g];}
//     }
//     std::cout<<"figure"<<figure<<"max"<<max<<std::endl; 

 ////  //为标签定方位
   
//  if(-0.785<hyp_pose_.pose_mean[2]&&hyp_pose_.pose_mean[2]<0.785)
//  {std::cout << "**1**" <<std::endl;
//    s_toward=1;
//  }
//  if(0.785<hyp_pose_.pose_mean[2]&&hyp_pose_.pose_mean[2]<2.355)
//  {std::cout << "**2**" <<std::endl;
//   s_toward=2; 
//  }
//  if(-2.355>hyp_pose_.pose_mean[2]&&hyp_pose_.pose_mean[2]>-3.141||2.355<hyp_pose_.pose_mean[2]&&hyp_pose_.pose_mean[2]<3.141)
//  {std::cout << "**3**" <<std::endl;
//     s_toward=3;
//  }
//  if(-2.355<hyp_pose_.pose_mean[2]&&hyp_pose_.pose_mean[2]<-0.785)
//  {std::cout << "**4**" <<std::endl;
//   s_toward=4;
//  }
//     
    
    
 //接收解算相机数据
 
 
    sight_x=0.001*(msg->data[0]);
    sight_z=0.001*(msg->data[1]);
    figure=msg->data[2];
    arfa=atan(sight_x/sight_z);
    sita=hyp_pose_.pose_mean[2];
    gama=sita-arfa;
    distance_vision=sqrt(sight_x*sight_x+sight_z*sight_z);
    
  amcl_sign_x=hyp_pose_.pose_mean[0]-distance_vision*cos(gama)-0.3*cos(hyp_pose_.pose_mean[2]);
  amcl_sign_y=hyp_pose_.pose_mean[1]-distance_vision*sin(gama)-0.3*sin(hyp_pose_.pose_mean[2]);
  
//    std::cout<<"*****"<<std::endl;
//         std::cout<<" amcl_sign_x="<<amcl_sign_x<<" amcl_sign_y="<<amcl_sign_y<<"figure="<<figure<<std::endl;
// 	std::cout<<" hy_x="<<hyp_pose_.pose_mean[0]<<"hy_y="<<hyp_pose_.pose_mean[1]<<std::endl;
// 	std::cout<<" -0.3*cos(hy_mean[2])"<<-0.3*cos(hyp_pose_.pose_mean[2])<<" -0.3*sin(hy_mean[2])="<<-0.3*sin(hyp_pose_.pose_mean[2])<<std::endl;
//         std::cout<<" -distance*cos(gama)="<<-distance_vision*cos(gama)<<" -distance*sin(gama)="<<-distance_vision*sin(gama)<<std::endl;
//    std::cout<<"*****"<<std::endl;
   
   
   ofstream gg;   //筛选合格坐标
   ofstream kk;   //基本变量
   ofstream kv;   //txt里标签坐标
    kk.open("/home/tdt/catkin_ws/src/RoboRTS/roborts_localization/kk.txt",ios::app);  //流写入位置
    kk<<"h_x   = "<<hyp_pose_.pose_mean[0]<<" h_y  = "<<hyp_pose_.pose_mean[1]<<" h_y_angle="<<hyp_pose_.pose_mean[2]<<endl
    <<  "amcl_x= " << amcl_sign_x <<" amcl_y= " <<amcl_sign_y<<endl
    <<  "distance_x="<<-distance_vision*cos(gama)<<" distance_y="<<-distance_vision*sin(gama)<<" figure="<<figure<<endl
    <<  "pcosx = " <<-0.3*cos(hyp_pose_.pose_mean[2])<< " psiny= " <<-0.3*sin(hyp_pose_.pose_mean[2])<<endl;
    kk.close();

 
//   int j,n;
//   int wr=0;
//  if(m<30)
//  {
//     for(j=0;j<30;j++)
//     {
//      er_x=MyConfig.PATROL_POINT[j].x-amcl_sign_x;
//      er_y=MyConfig.PATROL_POINT[j].y-amcl_sign_y;
//      
//     //std::cout << "m= " <<m<< " mx= " << MyConfig.PATROL_POINT[j].x <<" my= " <<  MyConfig.PATROL_POINT[j].y<<" myfigure= " <<  MyConfig.PATROL_POINT[j].z<<" myyaw= " <<  MyConfig.PATROL_POINT[j].yaw<< std::endl;
// 
//          if(-0.2<er_x&&er_x<0.2&&-0.2<er_y&&er_y<0.2)
//            {
// 	    std::cout << " 符合" <<std::endl;
//             //加一个for循环使得arry里元素不重复
// 	      for(n=0;n<m;n++)
// 	      {
//                 if(figure==arry[n][2])	     
// 	          {wr=0;break;}
//                 else{wr=1;}
//               }
// 
//               if((wr==1&&s_toward==MyConfig.PATROL_POINT[j].yaw)||m==0) // if(wr==1||m==0)      //改.yaw为.z 
// 	          {
// 		   std::cout<<"写入gg"<<std::endl;
// 	           arry[m][0]= MyConfig.PATROL_POINT[j].x;
// 	           arry[m][1]= MyConfig.PATROL_POINT[j].y;
// 	           arry[m][2]= figure;	
// 		   std::cout<<"*****"<<std::endl;
// 	             std::cout<<"m="<<m<<"arry[m][0]="<<arry[m][0]<<"arry[m][1]="<<arry[m][1]<<" arry[m][2]="<< arry[m][2]<<"mf="<<MyConfig.PATROL_POINT[j].z<<std::endl
// 	             <<"amcl_sign_x="<<amcl_sign_x<<"amcl_sign_y="<<amcl_sign_y<<"s_toward"<<s_toward<<std::endl;
// 		     std::cout<<"hy_x="<<hyp_pose_.pose_mean[0]<<"hy_y="<<hyp_pose_.pose_mean[1]<<" distance_x="<<-distance_vision*cos(gama)<<" distance_y="<<-distance_vision*sin(gama)<<"figure="<<figure<<std::endl;
//                    std::cout<<"*****"<<std::endl;
//                        gg.open("/home/tdt/catkin_ws/src/RoboRTS/roborts_localization/gg.txt",ios::app);  //流写入位置
//                        gg<<"m="<<m<<" er_x="<<er_x<<" er_y="<<er_y<<" arry[m][0]="<<arry[m][0]<<" arry[m][1]="<<arry[m][1]<<" amcl_sign_x="<<amcl_sign_x<< " amcl_sign_y"<<amcl_sign_y<<" arry[m][2]="<<endl
//                       <<"****arry[m][2]=****"<<arry[m][2]<<endl 
//                       <<"******myfigure=****"<<MyConfig.PATROL_POINT[j].z<<" s_toward="<<s_toward<<" M_yaw="<<MyConfig.PATROL_POINT[j].yaw<<"****"<<endl;  //流输出内容，用法和COUT一样
//                        gg.close(); 
// 	             m++;
// 	             break;
// 	           }
//              }      
//      }
//  }
//    } 
// // //   ///
// //   ///hahahahaahahah
//   ////
  
  
 
}


void LocalizationNode::PublishVisualize()
{
 
   if(pose_pub_.getNumSubscribers() > 0)
   {
    // std::cout<<"----------------------------------------------------------------"<<std::endl;
     
////////  
     
//         for(i=0;i<30;i++)
//         {
// 	 // std::cout<<"--------------------------------i = "<<i<<std::endl;
//           if(figure==arry[i][2])
//           {
//             hyp_pose_2.pose_mean[0]=arry[i][0]+distance_vision*cos(gama)+0.3*cos(hyp_pose_1.pose_mean[2]);
//             hyp_pose_2.pose_mean[1]=arry[i][1]+distance_vision*sin(gama)+0.3*sin(hyp_pose_1.pose_mean[2]);
// 	    
// // 	    std::cout<<" figure="<<figure<<" arry[i][2]="<<arry[i][2]<<std::endl;
// // 	    std::cout<<" vi_hy_x="<<hyp_pose_2.pose_mean[0]<<" vi_hy_y="<<hyp_pose_2.pose_mean[1]<<std::endl;
// // 	    std::cout<<" hy_x="<<hyp_pose_.pose_mean[0]<<" hy_y="<<hyp_pose_.pose_mean[1]<<std::endl;
// // 	    
// 	    ofstream hv;   //流名称
//               hv.open("/home/tdt/catkin_ws/src/RoboRTS/roborts_localization/hv.txt",ios::app);  //流写入位置
//               hv<<" vi_hy_x="<<hyp_pose_2.pose_mean[0]<<" vi_hy_y="<<hyp_pose_2.pose_mean[1]<<endl
//                 <<"    hy_x="<<hyp_pose_.pose_mean[0]<<"  hy_y="<<hyp_pose_.pose_mean[1]<<endl
//                 <<"arry[i][0]="<<arry[i][0]<<"arry[i][0]="<<arry[i][0]<<endl
//                 <<  "distance_x="<<-distance_vision*cos(gama)<<" distance_y="<<-distance_vision*sin(gama)<<endl
//                 <<  "pcosx = " <<-0.3*cos(hyp_pose_.pose_mean[2])<< " psiny= " <<-0.3*sin(hyp_pose_.pose_mean[2])<<endl
//                 <<"***********"<<endl;  //流输出内容，用法和COUT一样
//               hv.close();
// 	    
// 	    error_sign_x=hyp_pose_2.pose_mean[0]-hyp_pose_.pose_mean[0];
// 	    error_sign_y=hyp_pose_2.pose_mean[1]-hyp_pose_.pose_mean[1];
// 	    if(0.3<error_sign_x||error_sign_x<-0.3||0.3<error_sign_y||error_sign_y<-0.3)
// 	    {    
// 	         std::cout<<"!! !!!突变——漂了!!!!!"<<std::endl;
// 	         hyp_pose_1.pose_mean[0] = hyp_pose_2.pose_mean[0];
//                  hyp_pose_1.pose_mean[1] = hyp_pose_2.pose_mean[1]; 
// 		 break;
//              }
//              else
// 	     {
// 	       hyp_pose_1.pose_mean[0] = hyp_pose_.pose_mean[0];
//                hyp_pose_1.pose_mean[1] = hyp_pose_.pose_mean[1]; 
// 	     }
//           
//           }
//           else
// 	  {
// 	    hyp_pose_1.pose_mean[0] = hyp_pose_.pose_mean[0];
//             hyp_pose_1.pose_mean[1] = hyp_pose_.pose_mean[1]; 
// 	  }
// 	  std::cout<<"i="<<i<<std::endl;
//       }
    
    
    /////////不用
         //   hyp_pose_1.pose_mean[0] = hyp_pose_.pose_mean[0];
         //   hyp_pose_1.pose_mean[1] = hyp_pose_.pose_mean[1]; 
    /////////    
    pose_msg_.header.stamp = ros::Time::now();
    pose_msg_.header.frame_id = global_frame_;
    pose_msg_.pose.position.x = hyp_pose_1.pose_mean[0];
    pose_msg_.pose.position.y = hyp_pose_1.pose_mean[1];
    pose_msg_.pose.orientation = tf::createQuaternionMsgFromYaw(hyp_pose_.pose_mean[2]);
    pose_pub_.publish(pose_msg_);

    pose_msg_pose_position_x_last=pose_msg_.pose.position.x;
    pose_msg_pose_position_y_last=pose_msg_.pose.position.y;
  }

  if(particlecloud_pub_.getNumSubscribers() > 0){
    particlecloud_msg_.header.stamp = ros::Time::now();
    particlecloud_msg_.header.frame_id = global_frame_;
    particlecloud_pub_.publish(particlecloud_msg_);
  }

  if(!publish_first_distance_map_) {
    distance_map_pub_.publish(amcl_ptr_->GetDistanceMapMsg());
    publish_first_distance_map_ = true;
  }
}

bool LocalizationNode::PublishTf()
{
  static int niu=0;
  int i;
  float error_sign_x,error_sign_y;
  
  ros::Time transform_expiration = (last_laser_msg_timestamp_ + transform_tolerance_);
  if (amcl_ptr_->CheckTfUpdate()) 
  {  
    // Subtracting base to odom from map to base and send map to odom instead
    tf::Stamped<tf::Pose> odom_to_map;


    ////////hhhhhhhhhhhhhhhhaahahahahaha
    for(i=0;i<30;i++)
    {
      //std::cout<<"--------------i = "<<i<<std::endl;
      //std::cout<<"arry["<<i<<"][2] = "<<arry[i][2]<<std::endl;
      if(figure==MyConfig.PATROL_POINT[i].z)
      {
	std::cout<<"------------匹配成功--------"<<MyConfig.PATROL_POINT[i].z<<"------"<<figure<<std::endl;
        hyp_pose_2.pose_mean[0]=MyConfig.PATROL_POINT[i].x+distance_vision*cos(gama)+0.3*cos(hyp_pose_1.pose_mean[2]);
        hyp_pose_2.pose_mean[1]=MyConfig.PATROL_POINT[i].y+distance_vision*sin(gama)+0.3*sin(hyp_pose_1.pose_mean[2]);
        std::cout<<" figure="<<figure<<" MyConfig.PATROL_POINT[i].z="<<MyConfig.PATROL_POINT[i].z<<std::endl;
      std::cout<<" hyp_pose_2_x="<<hyp_pose_2.pose_mean[0]<<" hyp_pose_2_y="<<hyp_pose_2.pose_mean[1]<<std::endl;
      //std::cout<<" hy_x="<<hyp_pose_.pose_mean[0]<<" hy_y="<<hyp_pose_.pose_mean[1]<<std::endl;
        
        error_sign_x=hyp_pose_2.pose_mean[0]-hyp_pose_.pose_mean[0];
        error_sign_y=hyp_pose_2.pose_mean[1]-hyp_pose_.pose_mean[1];
        if(1.0<error_sign_x||error_sign_x<-1.0||1.0<error_sign_y||error_sign_y<-1.0)
        {    niu++;
          std::cout<<"!! !!!机器人迷失!!!!!   ="<<niu<<std::endl;
//写入漂了位姿	  
// 	  ofstream hv;   //流名称
//           hv.open("/home/tdt/catkin_ws/src/RoboRTS/roborts_localization/hv.txt",ios::app);  //流写入位置
//           hv<<"vi_hy_x="<<hyp_pose_2.pose_mean[0]<<"  hy_x="<<hyp_pose_.pose_mean[0]<<endl
//             <<"vi_hy_y="<<hyp_pose_2.pose_mean[1]<<"  hy_y="<<hyp_pose_.pose_mean[1]<<endl
//             <<"读标签figure="<<figure<<" 找hy坐标附近对应标签re.txt3"<<endl
//             <<"标签位姿arry[i][0]="<<arry[i][0]<<"arry[i][1]="<<arry[i][1]<<endl
//             <<"distance_x="<<distance_vision*cos(gama)<<" distance_y="<<distance_vision*sin(gama)<<endl
//             <<"pcosx = " <<0.3*cos(hyp_pose_.pose_mean[2])<<" psiny= "<<0.3*sin(hyp_pose_.pose_mean[2])<<endl
//             <<"***********"<<endl;  //流输出内容，用法和COUT一样
//           hv.close();
	  
          hyp_pose_1.pose_mean[0] = hyp_pose_2.pose_mean[0];
          hyp_pose_1.pose_mean[1] = hyp_pose_2.pose_mean[1]; 
        break;
        }
        else
        {
          std::cout<<"---------------------------------------"<<std::endl;
          hyp_pose_1.pose_mean[0] = hyp_pose_.pose_mean[0];
          hyp_pose_1.pose_mean[1] = hyp_pose_.pose_mean[1]; 
        }
      }
      else
      {
        std::cout<<"***********************"<<std::endl;
        hyp_pose_1.pose_mean[0] = hyp_pose_.pose_mean[0];
        hyp_pose_1.pose_mean[1] = hyp_pose_.pose_mean[1]; 
      }
    //  std::cout<<"i="<<i<<std::endl;
    } 



    ///////////////////////////////////////////////////
     //tf转换
    try {
      std::cout<<"hyp_pose_.pose_mean[2] is :"<<hyp_pose_.pose_mean[2]<<std::endl;
      std::cout<<"hyp_pose_.pose_mean[1] is :"<<hyp_pose_.pose_mean[1]<<std::endl;
      std::cout<<"hyp_pose_.pose_mean[0] is :"<<hyp_pose_.pose_mean[0]<<std::endl;
      tf::Transform tmp_tf(tf::createQuaternionFromYaw(hyp_pose_.pose_mean[2]),
                           tf::Vector3(hyp_pose_1.pose_mean[0],
                                       hyp_pose_1.pose_mean[1],
                                       0.0));//hyp_pose_1.pose_mean[0]
     
      
      tf::Stamped<tf::Pose> tmp_tf_stamped(tmp_tf.inverse(),
                                           last_laser_msg_timestamp_,
                                           base_frame_);
      this->tf_listener_ptr_->transformPose(odom_frame_,
                                            tmp_tf_stamped,
                                            odom_to_map);
    } catch (tf::TransformException &e) {
      LOG_ERROR << "Failed to subtract base to odom transform" << e.what();
      return false;
    }

    latest_tf_ = tf::Transform(tf::Quaternion(odom_to_map.getRotation()),
                               tf::Point(odom_to_map.getOrigin()));
    latest_tf_valid_ = true;

    tf::StampedTransform tmp_tf_stamped(latest_tf_.inverse(),
                                        transform_expiration,
                                        global_frame_,
                                        odom_frame_);
    this->tf_broadcaster_ptr_->sendTransform(tmp_tf_stamped);
    sent_first_transform_ = true;
    return true;
  } 
  else if (latest_tf_valid_) 
  {
    // Nothing changed, so we'll just republish the last transform
    tf::StampedTransform tmp_tf_stamped(latest_tf_.inverse(),
                                        transform_expiration,
                                        global_frame_,
                                        odom_frame_);
    this->tf_broadcaster_ptr_->sendTransform(tmp_tf_stamped);
    return true;
  }
  else{
    return false;
  }
}

bool LocalizationNode::GetPoseFromTf(const std::string &target_frame,
                   const std::string &source_frame,
                   const ros::Time &timestamp,
                   Vec3d &pose)
{
  tf::Stamped<tf::Pose> ident(tf::Transform(tf::createIdentityQuaternion(),
                                            tf::Vector3(0, 0, 0)),
                              timestamp,
                              source_frame);
  tf::Stamped<tf::Pose> pose_stamp;
  try {
    this->tf_listener_ptr_->transformPose(target_frame,
                                          ident,
                                          pose_stamp);
  } 
  catch (tf::TransformException &e) 
{
    LOG_ERROR << "Couldn't transform from "
              << source_frame
              << "to "
              << target_frame;
    return false;
  }

  pose.setZero();
  pose[0] = pose_stamp.getOrigin().x();
  pose[1] = pose_stamp.getOrigin().y();
  double yaw,pitch, roll;
  pose_stamp.getBasis().getEulerYPR(yaw, pitch, roll);
  pose[2] = yaw;
  return true;
}

void LocalizationNode::TransformLaserscanToBaseFrame(double &angle_min,
                                                     double &angle_increment,
                                                     const sensor_msgs::LaserScan &laser_scan_msg) {

  // To account for lasers that are mounted upside-down, we determine the
  // min, max, and increment angles of the laser in the base frame.
  // Construct min and max angles of laser, in the base_link frame.
  tf::Quaternion q;
  q.setRPY(0.0, 0.0, laser_scan_msg.angle_min);
  tf::Stamped<tf::Quaternion> min_q(q, laser_scan_msg.header.stamp,
                                    laser_scan_msg.header.frame_id);
  q.setRPY(0.0, 0.0, laser_scan_msg.angle_min
      + laser_scan_msg.angle_increment);
  tf::Stamped<tf::Quaternion> inc_q(q, laser_scan_msg.header.stamp,
                                    laser_scan_msg.header.frame_id);

  try {
    tf_listener_ptr_->transformQuaternion(base_frame_,
                                          min_q,
                                          min_q);
    tf_listener_ptr_->transformQuaternion(base_frame_,
                                          inc_q,
                                          inc_q);
  }
  catch (tf::TransformException &e) {
    LOG_WARNING << "Unable to transform min/max laser angles into base frame: " << e.what();
    return;
  }

  angle_min = tf::getYaw(min_q);
  angle_increment = (tf::getYaw(inc_q) - angle_min);

  // Wrapping angle to [-pi .. pi]
  angle_increment = (std::fmod(angle_increment + 5 * M_PI, 2 * M_PI) - M_PI);

}

}// roborts_localization

int main(int argc, char **argv) {
  roborts_localization::GLogWrapper glog_wrapper(argv[0]);
  ros::init(argc, argv, "localization_node");
   //  std::cout<<"00000"<<std::endl;

  roborts_localization::LocalizationNode localization_node("localization_node");
  
     //  std::cout<<"010101"<<std::endl;

  ros::AsyncSpinner async_spinner(THREAD_NUM);
  async_spinner.start();
  ros::waitForShutdown();
  return 0;
}
