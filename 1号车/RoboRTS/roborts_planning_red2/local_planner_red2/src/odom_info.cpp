...
void OdomInfo::OdomCB(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  //ROS_ERROR_STREAM("odom_.child_frame_id is : "<<odom_.child_frame_id);
}
...
void OdomInfo::GetVel(tf::Stamped<tf::Pose>& vel) {
  geometry_msgs::Twist temp_vel;{
    ...
    //ROS_ERROR_STREAM("vel.frame_id_  is : "<<vel.frame_id_ );
  }
...
void OdomInfo::SetTopic(std::string topic)
{
  if (topic_ == topic) {
    ...
    if (topic == "") {
      ...
      ros::NodeHandle nh;
      //ROS_ERROR_STREAM("topic_ is : "<<topic_);
      sub_ = nh.subscribe<nav_msgs::Odometry>("/red_car2/odom", 1, boost::bind( &OdomInfo::OdomCB, this, _1 ));
    }
  }
}
