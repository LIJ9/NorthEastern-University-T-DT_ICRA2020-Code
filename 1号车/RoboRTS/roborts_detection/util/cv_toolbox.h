...
class CVToolbox {
 public:
   ...
   */
  explicit CVToolbox(std::string camera_name,unsigned int buffer_size = 3) : get_img_info_(false)
  {  
...
//      ROS_WARN("capture time: %lf", capture_time_);
    ...
    for (int i = 0; i < buffer_state_.size(); ++i) {
      if (buffer_state_[i] != BufferState::READ) {
        ...
        lock_.lock();
        latest_index_ = i;
	//std::cout<<"latest_index_"<<latest_index_<<std::endl;
        lock_.unlock();
      }
    }
    /*
...
//---------------------------------------[ADD]----------------------------------------// 
  ...
  {
    std::vector<std::vector<cv::Point>> contours;
    const auto mode = CV_RETR_EXTERNAL;//检测所有外轮廓
    const auto method = CV_CHAIN_APPROX_SIMPLE;//仅将拐点处的点存放到contours
...
//---------------------------------------[END]---------------------------------------//
 ...
 private:
  std::vector<cv::Mat> image_buffer_; //图像缓存区
  std::vector<BufferState> buffer_state_;//缓存区的状态
  int latest_index_;
  std::mutex lock_;
  int index_;
