...
GlobalPlannerNode::GlobalPlannerNode() :
    new_path_(false),pause_(false), node_state_(NodeState::IDLE), error_info_(ErrorCode::OK),/*goal_callback_info[0]=0,goal_callback_info[1] =0,*/
...
ErrorInfo GlobalPlannerNode::Init() {
  ...
  goal_pub_ = goal_h.advertise<geometry_msgs::PoseStamped>("current_goal", 1);
...
void GlobalPlannerNode::GoalCallback(const roborts_msgs::GlobalPlannerGoal::ConstPtr &msg) {
  ...
  //goal_callback_pub_ = goal_callback_h.advertise< std_msgs::Float64MultiArray>("/goal_callback", 10);
  ...
  while (ros::ok()) {
    ...
    if(node_state == NodeState::RUNNING || node_state == NodeState::SUCCESS || node_state == NodeState::FAILURE|| node_state == NodeState::STUCK) {
...
void GlobalPlannerNode::PlanThread() {
  ...
  ros::NodeHandle n;
 ros::Publisher point_pub_ = n.advertise<geometry_msgs::PoseStamped >("/stuck_pose", 10);
  ...
  while (ros::ok()) {   
   ...
   std::unique_lock<std::mutex> plan_lock(plan_mutex_);
	...
	SetNodeState(NodeState::SUCCESS);
      ...
      planthread.pose.position.y = 1;
  
      ROS_ERROR("STUCK!!Can not get plan with max retries( %d )", max_retries_ );         
      error_info = ErrorInfo(ErrorCode::GP_MAX_RETRIES_FAILURE, "Over max retries.");   
      SetNodeState(NodeState::STUCK);//FAILURE);
      point_pub_.publish(current_start); 
      retries=0;
   
    } else if (error_info == ErrorInfo(ErrorCode::GP_GOAL_INVALID_ERROR)){  /////目标点不可用     
   
       planthread.pose.position.z = 1;
      ...
      retries++; 
       planthread.pose.orientation.x= 1;
      ROS_ERROR("Can not get plan for once. %s", error_info.error_msg().c_str());    
      SetErrorInfo(ErrorInfo(ErrorCode::GP_PATH_SEARCH_ERROR, "GP_PATH_SEARCH_ERROR"));////////////  第一句 condition2,6次
    ...
    if (sleep_time <= std::chrono::microseconds(0)) {
      //     plan_thread_info.data[7] = 1000+ cycle_duration_.count();
        planthread.pose.orientation.y= 1;
   ...
   planthread_pub.publish(planthread);
