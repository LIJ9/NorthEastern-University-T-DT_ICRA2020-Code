...
class AStarPlanner : public GlobalPlannerBase {
 ...
 void  areastatusCallback(const std_msgs::Float64MultiArray::ConstPtr &msg);
 void  currentgoalCallback(const geometry_msgs::PoseStamped::ConstPtr &goal_msg);
     
   void changegoal_PosCallback(const std_msgs::Float64::ConstPtr& msg);
   
   
     std::chrono::system_clock::time_point oscillation_;
  //! Time allow robot oscillation at a position
  double oscillation_time_;
  int last_osc_dist,osc_dist;
   
   
   
 private:
   //////////////////////
      ros::NodeHandle goal_h;
   ros::Subscriber goal_sub;
     ros::NodeHandle h;
   ros::Subscriber area_sub_g;
   int area[6];
    float F[6];
    int last_test_dist,current_dist,sum = 0/*,ice_no = 0*/;
    geometry_msgs::PoseStamped current_goal,current_position ;
   int current_x,current_y , first_index;
   unsigned int last_index= 0;
   
       ros::NodeHandle osci_h;
  ros::Subscriber osci_pose_sub;// =osci_h.subscribe("stuck_pos", 10,& AStarPlanner::osci_PosCallback,this);
 bool is_osci;

 ///////////////
   ros::NodeHandle changegoal_h;
  ros::Subscriber changegoal_sub;
      double changegoal;
      bool if_changed =false;
      
       
    ros::NodeHandle Aplan_h;
    ros::Publisher Aplan_pub ;//= Aplan_h.advertise<Float64MultiArray>("/Aplan_info", 10);

    
        ros::NodeHandle insert_pos_h;
    ros::Publisher insert_pos_pub; // = insert_pos_h.advertise<std_msgs::Float64MultiArray>("/insert_pose", 1);
 geometry_msgs::PoseStamped/*std_msgs::Float64MultiArray*/ insert_pos; 
 
   /*    ros::NodeHandle searchpath_h;
    //! ROS Publisher for current goal
    ros::Publisher searchpath_pub_;//searchpath_pub_ = searchpath_h.advertise<Float64>("/searchpath", 10);
     std_msgs::Float64 searchpath_info;
  ...
  static std::vector<int> f_score_,last_f_score;
