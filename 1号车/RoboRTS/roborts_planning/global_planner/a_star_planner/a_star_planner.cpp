...
AStarPlanner::AStarPlanner(CostmapPtr costmap_ptr) :
  ...
  std::string full_path = ros::package::getPath("roborts_planning") + "/global_planner/a_star_planner/"\
     ...
     insert_pos_pub = insert_pos_h.advertise<geometry_msgs::PoseStamped>("/insert_goal", 1);
...
ErrorInfo AStarPlanner::Plan(const geometry_msgs::PoseStamped &start,
  ...
  unsigned int start_x, start_y, goal_x, goal_y, tmp_goal_x, tmp_goal_y/*,tpx,tpy*/;
...
//     Aplan_pub_.publish(Aplan_info);
   insert_pos.pose.position.x= 0;
   insert_pos.pose.position.y= 0;
   insert_pos.pose.position.z= 0;
   insert_pos.pose.orientation.x= 0;
   insert_pos.pose.orientation.y= 0;
   insert_pos.pose.orientation.z= 0;
  if (costmap_ptr_->GetCostMap()->GetCost(goal_x,goal_y)>=inaccessible_cost_) {

    unsigned int tolerance;
    if(goal_x>5&&goal_x<16&&goal_y>51&&goal_y<61||goal_x>30&&goal_x<49&&goal_y>24&&goal_y<38||goal_x>76&&goal_x<87&&goal_y>77&&goal_y<86){
      tolerance = 0;
      insert_pos.pose.position.x= 5.0;
      insert_pos.pose.position.y= 1.6;
      insert_pos.pose.position.z= 1.0;
      insert_pos.pose.orientation.w= 1.0;
      ROS_ERROR(" Insert a goal 5,1.6, -x");
       insert_pos_pub.publish(insert_pos);
       
    }
   else if(goal_x>147&&goal_x<158&&goal_y>29&&goal_y<39||goal_x>119&&goal_x<130&&goal_y>52&&goal_y<62||goal_x>76&&goal_x<87&&goal_y>4&&goal_y<14){
     insert_pos.pose.position.x= 3.0;
     insert_pos.pose.position.y= 2.6;
     insert_pos.pose.position.z= 2.0;
     insert_pos.pose.orientation.w= 1.0;
     tolerance = 0;
     ROS_ERROR(" Insert a goal 3,2.6,+x");
     insert_pos_pub.publish(insert_pos);
  }
  else
  {  
    ROS_ERROR("Changed A Goal!");
    tolerance =0.7;
      }
    
      
     if(costmap_ptr_->GetCostMap()->GetCost(goal_x - tolerance,goal_y - tolerance)<inaccessible_cost_)  //8
     {
      tmp_goal_y = goal_y - tolerance;
      tmp_goal_x  = goal_x - tolerance;   
       
    }
    else if(costmap_ptr_->GetCostMap()->GetCost(goal_x ,goal_y - tolerance)<inaccessible_cost_){                     //1

        tmp_goal_y = goal_y - tolerance ;
       tmp_goal_x = goal_x;
    }
    else if(costmap_ptr_->GetCostMap()->GetCost(goal_x - tolerance,goal_y)<inaccessible_cost_)                       //7
   {
      tmp_goal_y = goal_y;
      tmp_goal_x =  goal_x - tolerance ;
    }else if(costmap_ptr_->GetCostMap()->GetCost( goal_x - tolerance, goal_y + tolerance)<inaccessible_cost_){
      tmp_goal_y = goal_y + tolerance;
       tmp_goal_x = goal_x - tolerance;
    }else if(costmap_ptr_->GetCostMap()->GetCost(goal_x + tolerance ,goal_y - tolerance)<inaccessible_cost_) {//2
        tmp_goal_y = goal_y - tolerance;
        tmp_goal_x = goal_x + tolerance;
    }
    else if(costmap_ptr_->GetCostMap()->GetCost(goal_x ,goal_y + tolerance )<inaccessible_cost_){                           //5
        tmp_goal_y = goal_y + tolerance;
        tmp_goal_x = goal_x ;
    }else if(costmap_ptr_->GetCostMap()->GetCost(goal_x + tolerance, goal_y )<inaccessible_cost_){                    //3 
        tmp_goal_y =  goal_y ;
        tmp_goal_x =  goal_x + tolerance;
    }else  if(costmap_ptr_->GetCostMap()->GetCost(goal_x + tolerance,goal_y + tolerance)<inaccessible_cost_){//4
        tmp_goal_y = goal_y + tolerance;
        tmp_goal_x = goal_x + tolerance;
    }
  ...
  if (!goal_valid){
    ...
    unsigned int start_index, goal_index,last_start_index;
    ...
    if(start_index == goal_index){//目标点有效reached
...
ErrorInfo AStarPlanner::SearchPath(const int &start_index,
  ...
  std::vector<int> neighbors_index;
   int current_index, move_cost, h_score, count = 0,h_parent;
  ...
  while (!openlist.empty()) {
    ...
    for (auto neighbor_index : neighbors_index) {
	 ...
	 GetManhattanDistance(current_index, goal_index, h_parent);
	    
          f_score_.at(neighbor_index) = g_score_.at(neighbor_index) + h_score+h_parent;
