...
class Blackboard {
 public:
  ...
  {    
    lidar_enemy_pose_.pose.position.x= msg->pose.position.x;
    lidar_enemy_pose_.pose.position.y= msg->pose.position.y;
    if(lidar_enemy_pose_.pose.position.x <0&&lidar_enemy_pose_.pose.position.y <0)
    {
      lidar_enemy_detected_ = false;//误识别
      return;
    }
...
//     Lidardistance_ ==std::sqrt(std::pow(dx, 2) + std::pow(dy, 2));
//     if( Lidardistance_ <= 3 ) 
//     {
...
//     } 
  ...
  lidar_yaw_ = atan2( (lidar_enemy_pose_.pose.position.y - robot_map_pose.pose.position.y), (lidar_enemy_pose_.pose.position.x - robot_map_pose.pose.position.x) );
    ...
    //ROS_ERROR("lidar_enemy_pose_.pose.position.x = %lf",lidar_enemy_pose_.pose.position.x);
    //lidar_enemy_detected_ = true;
  }
  ...
  double EnemyDistance()
  {
    ...
    double LidarEnemyDistance()
  {
    return Lidardistance_;
  }
...
//my position
 ...
 private:
  ...
  //enemy distance
  double distance_,Lidardistance_;
