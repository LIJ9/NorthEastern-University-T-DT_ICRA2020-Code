...
double laser;
bool adjust_sign=false;/*************底盘要是控制不了，看看这里，正常是false************/
double degree=0;
...
void sendCallback(const sensor_msgs::LaserScan& msg)//right
{
  ...
  diff_x=(angle[0]+offset_front_x*tan(ans[0]))*cos(ans[0])-0.5;
  diff_y=(laser+offset_back_x)*cos(ans[0])-0.5;//距底面,单位m
}
...
{
  sight_x=-(msg->data[0]);
  sight_y=-(msg->data[1]-300);
 // ROS_INFO_STREAM("sight_x="<<sight_x<<"sight_y="<<sight_y);
}
double sight_i_x,sight_i_y;
double comout_x,comout_y;//视觉标签最终四轮的速度输出
int main(int argc,char** argv)
{
  ...
  //ros::Subscriber marker_info_sub = n.subscribe("marker_Point3d",1000,marker_Callback);
  ros::Subscriber vosion_location_sub = n.subscribe("vision_tolocation",1,vision_location_Callback);
  ROS_INFO_STREAM("SUB_OVER");
  ...
  while(ros::ok())
  {
    ...
    if (adjust_sign==true)
    {
      ...
      // // cmd_pose.linear.x=-limit(out_v_y,1,-1);
      ...
      // // cmd_pose.linear.y=limit(out_v_x,1,-1);
      ...
      sight_i_x += sight_x;
      comout_x=sight_x*0.003;//+sight_i_x*0.0004;
     //ROS_INFO_STREAM("comout_x="<<comout_x);
      cmd_pose.linear.y=-limit(comout_x,0.6,-0.6);

      sight_i_y += sight_y;
      comout_y=sight_y*0.002;//+sight_i_y*0.4;
      cmd_pose.linear.x=limit(comout_y,0.6,-0.6);
