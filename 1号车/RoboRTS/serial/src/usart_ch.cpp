...
*@datalen:数据长度
*/
int Class_Usart::Usart_Send(short head,int my_position_x,int my_position_y,int enemy_position_x,int enemy_position_y,int bufflet,bool bufflet_sign,short buff_sign,int remain_blood)
{
    int len = 0;
    ...
    buff[7]=(static_cast<short>((int)enemy_position_y))>>8;;
    buff[8]=(static_cast<short>((int)enemy_position_y))&0x00ff;
    buff[9]=(static_cast<short>((int)bufflet))>>8;;
    buff[10]=(static_cast<short>((int)bufflet))&0x00ff;
    buff[11]=static_cast<char>(bufflet_sign);
    buff[12]=static_cast<char>(buff_sign);
    buff[13]=(static_cast<short>((int)remain_blood))>>8;;
    buff[14]=(static_cast<short>((int)remain_blood))&0x00ff;
...
//printf("%x,%x,%x,%x,%x,%x,%x,%x,%x;\n", buff[0], buff[1], buff[2],buff[3], buff[4], buff[5],buff[6], buff[7], buff[8]);
    ...
    //std::cout<<"aaaaaaaaaaaaaaaaaaaaaaa"<<total_send_length<<std::endl;
    len = write(Class_Usart::serial_fd, buff,total_send_length );//实际写入的长度
    //std::cout<<"aaaaaaaaaaaaaaaaaaaaaaa"<<total_send_length<<"aaaaaaaaaa"<<len<<std::endl;
#ifdef DEBUG
